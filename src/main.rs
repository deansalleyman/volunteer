    ...
    match fs::metadata(file_path) {
        Ok(metadata) => {
            if metadata.is_file() {
                ...
                loop {
                println!("Please choose your volunteer. Press Enter or type 'quit' to exit.");
                ...
                match cmd.trim(){
                    "index" => println!("Index: {volunteer_number}"),
                    "quit" => {break;},
                    _ => println!("#{}", volunteer_number),
                }

                let user = &users[volunteer_number];
                println!("Today's volunteer is: {} {}", user.first_name, user.last_name)
                }
            } else {
